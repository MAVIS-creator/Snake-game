...
const bloom = new UnrealBloomPass(new THREE.Vector2(window.innerWidth,window.innerHeight),0.85,0.6,0.85);
...
const borderMesh = new THREE.LineSegments(edges, new THREE.LineBasicMaterial({color:0x00ffff}));
...
function drawSnake(){
    ...
    snake.forEach((seg,i)=>{
        ...
        m.position.set(seg.x,yLift,seg.y);
        scene.add(m);
        snakeMeshes.push(m);
    });
}
function drawFood(){
    ...
    foodMesh.position.set(food.x,yLift,food.y);
    scene.add(foodMesh);
}
...
function step(){
    ...
    // collisions
    if(sx<0 || sy<0 || sx>=N || sy>=N || snake.slice(1).some(s=>s.x===sx && s.y===sy)){
        isPaused = true;
...
function resetGame(){
    snake=[{x:9,y:10}]; direction="RIGHT"; score=0; level=1; stepMs=baseSpeed;
    food=randomFood(); obstacles=[];
